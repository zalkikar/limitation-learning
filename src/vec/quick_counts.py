...
lines = []
with open('./dat/processed/formatted_movie_lines.txt', 'r', encoding='utf-8') as out:
        for line in out:
            line = line.replace("\n", "")
            lines.append(
                line.replace(
...
plt.savefig('./src/vis/plots/formatted_movie_lines_spacy_token_counts.png')
plt.close()


plt.title('Per-Line Vectorized Token Counts for Movie Lines')
plt.hist(line_token_with_vector_lengths, bins=20)
plt.savefig('./src/vis/plots/formatted_movie_lines_spacy_token_with_vector_counts.png')
plt.close()
...
plt.legend()
plt.savefig('./src/vis/plots/formatted_movie_lines_spacy_token_counts_compare.png')
plt.close()
